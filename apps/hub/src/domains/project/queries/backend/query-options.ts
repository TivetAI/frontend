...
export const projectBackendEnvVariablesQueryOptions = ({
	...
	queryOptions({
		...
		queryFn: ({ queryKey: [_, projectId, __, environmentId], signal }) =>
			tivetEeClient.ee.backend.getVariables(projectId, environmentId, {
				abortSignal: signal,
			}),
...
export const projectBackendEnvEventsQueryOptions = ({
	...
	queryOptions({
		...
		queryFn: async ({
			...
			const response = await tivetEeClient.ee.backend.getEvents(
				projectId,
				environmentId,
...
export const projectBackendProjectEnvDatabaseQueryOptions = ({
	...
	queryOptions({
		...
		queryFn: ({ queryKey: [_, projectId, __, environmentId] }) =>
			tivetEeClient.ee.backend.getDbUrl(projectId, environmentId),
	});
