...
import { getConfig, toast } from "@tivet-gg/components";
...
export const useCreateBackendMutation = ({
	...
	onSuccess?: (data: TivetEe.ee.backend.CreateResponse) => void;
}) =>
	useMutation({
		mutationFn: ({
			projectId,
			environmentId,
			...data
		}: TivetEe.ee.backend.CreateRequest & {
			projectId: string;
			environmentId: string;
		}) => tivetEeClient.ee.backend.create(projectId, environmentId, data),
...
export const useBackendUpdateVariablesMutation = () =>
	useMutation({
		mutationFn: ({
			...
			environmentId,
			...data
		}: TivetEe.ee.backend.UpdateVariablesRequest & {
			projectId: string;
			environmentId: string;
		}) =>
			tivetEeClient.ee.backend.updateVariables(
				projectId,
				environmentId,
				data,
			),
