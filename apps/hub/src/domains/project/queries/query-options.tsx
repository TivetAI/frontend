...
import { loadModuleCategories } from "@tivet-gg/components";
...
export const projectsQueryOptions = () => {
	return queryOptions({
		queryKey: ["projects"],
		queryFn: ({ meta, signal }) =>
			tivetClient.cloud.games.getGames(
				{
...
export const groupProjectsQueryOptions = (groupId: string) => {
	return queryOptions({
		...
		retry(failureCount, error) {
			if (isTivetError(error) && error.statusCode === 404) {
				return false;
			}
			return failureCount < 3;
		},
		queryFn: async ({ signal, meta }) => {
			const data = await tivetClient.cloud.games.getGames(
				{
					watchIndex: getMetaWatchIndex(meta),
				},
				{ abortSignal: signal },
...
export const projectQueryOptions = (projectId: string) => {
	return queryOptions({
		...
		queryFn: ({ queryKey: [_, projectId], signal, meta }) =>
			tivetClient.cloud.games.getGameById(
				projectId,
				{
					watchIndex: getMetaWatchIndex(meta),
				},
...
export const projectTokenCloudQueryOptions = ({
	...
	return queryOptions({
		...
		}) => tivetClient.cloud.games.tokens.createCloudToken(projectId),
...
export const projectEnvTokenServiceQueryOptions = ({
	...
	return queryOptions({
		...
		queryFn: ({
			...
			tivetClient.games.environments.tokens.createServiceToken(
				projectId,
				environmentId,
			),
		select: (data) => data.token,
...
export const projectMetadataQueryOptions = ({
	...
	return queryOptions({
		...
		queryFn: async ({ queryKey: [_, projectId, __, environmentId] }) => {
			const { game } =
				await tivetClient.cloud.games.getGameById(projectId);
			const legacyLobbiesEnabled = game.versions.length > 1;

			const bootstrap = await tivetClient.cloud.bootstrap();
			try {
				if (bootstrap.cluster === "enterprise") {
					// should throw when there is no backend
					await tivetEeClient.ee.backend.get(
						projectId,
						environmentId,
					);
					...
					const { variables } =
						await tivetEeClient.ee.backend.getVariables(
							projectId,
							environmentId,
						);
