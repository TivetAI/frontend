...
import { Flex, Grid, H2, Link } from "@tivet-gg/components";
import {
	faBadgeCheck,
	faCheckCircle,
	faClock,
...
interface BillingPlansProps {
	...
	config?: Partial<
		Record<TivetEe.ee.billing.Plan, Partial<BillingPlanCardProps>>
	>;
}
	...
	return (
		<>
			...
			<Grid columns={{ initial: "1", xl: "4" }} gap="4">
				<BillingPlanCard
					title="Community"
					price={`$${PRICE_MAP[TivetEe.ee.billing.Plan.Trial]}`}
					onSubscribe={() =>
						open({
							plan: TivetEe.ee.billing.Plan.Trial,
						})
					}
					...
					{...config?.[TivetEe.ee.billing.Plan.Trial]}
				/>
				<BillingPlanCard
					title="Pro"
					onSubscribe={() =>
						open({
							plan: TivetEe.ee.billing.Plan.Indie,
						})
					}
					onCancel={() =>
						open({
					...
					price={`$${PRICE_MAP[TivetEe.ee.billing.Plan.Indie]}`}
					type={
						plan === TivetEe.ee.billing.Plan.Indie
							? "active"
							: undefined
					...
					{...config?.[TivetEe.ee.billing.Plan.Indie]}
				/>
				<BillingPlanCard
					title="Team"
					onSubscribe={() =>
						open({
							plan: TivetEe.ee.billing.Plan.Studio,
						})
					}
					onCancel={() =>
						open({
					...
					price={`$${PRICE_MAP[TivetEe.ee.billing.Plan.Studio]}`}
					type={
						plan === TivetEe.ee.billing.Plan.Studio
							? "active"
							: undefined
					...
					{...config?.[TivetEe.ee.billing.Plan.Studio]}
				/>
				<BillingPlanCard
					title="Enterprise"
					price="Custom"
